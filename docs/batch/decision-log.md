...
## 2026-05-30: File Ingestion Jobs Are Rerun-Type (not restart-type)

Context: Phase 5 added opt-in IO value-adds — configurable charset, input-file lifecycle (`FileIngestionLifecycleListener`: existence/empty check + archive on success / move-to-error on failure), and atomic output (`AtomicOutputListener`: promote temp→final on success, discard temp otherwise). A final review surfaced how these interact with restart.

Decision: file ingestion jobs target the **rerun-from-scratch** model (resourceless `JobRepository`), not Spring Batch restart-from-failure. The lifecycle/atomic listeners move the input to the error directory and discard the output temp on any non-`COMPLETED` outcome; the input is consumed (archived/errored) per run. Restart-from-failure would then find the input missing and the writer's saved state inconsistent with a discarded temp file.

Reason: moving the input and finalizing output are naturally tied to a single end-to-end run. Coupling them to restart would require "move/promote only on final success or abandonment" semantics and reconciliation of partial output — heavier than the current need. The reference `billing-file` job runs resourceless, so the model is self-consistent.

Impact: the IO listeners are documented as rerun-type. A future restart-capable file job must redesign post-processing (defer moves to final outcome, reconcile partial output) before enabling JDBC restart. Related future considerations (input delivery-completion detection via sentinel/stable-size, deterministic output newline/BOM policy, parse-error classification 20-vs-30, `STOPPED` handling) are recorded in [docs/plans/80-io-support.md](../plans/80-io-support.md).
